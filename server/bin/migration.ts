...
function rotateDecksFromStandard(){
...
function updateToVersion24() {
    const version = 24;

    console.log(`apply ver${version}, repopulate cards to include Witchwood`);
    return cardDB.remove({}).exec()
        .then(() => parser.populateWithCards())
        .then(() => console.log(`apply ver${version}`));
}

function updateToVersion25() {
    const version = 25;

    console.log(`apply ver${version}, redo repopulate cards to include Witchwood`);
    return cardDB.remove({}).exec()
        .then(() => parser.populateWithCards())
        .then(() => console.log(`apply ver${version}`));
}
...
function updateToVersion26() {
    ...
    return rotateDecksFromStandard()
        .then(() => console.log(`ver${version} appplied successfully`));
}
...
function updateToVersion28() {
    const version = 28;

    console.log(`apply ver${version}, redo repopulate cards to include Boomsday`);
    return cardDB.remove({}).exec()
        .then(() => parser.populateWithCards())
        .then(() => console.log(`apply ver${version}`));
}

function updateToVersion29() {
    const version = 29;
...
function updateToVersion34() {
    const version = 34;

    console.log(`apply ver${version}, rotate decks from Standard `);
    return rotateDecksFromStandard()
        .then(() => console.log(`apply ver${version}`));
}
